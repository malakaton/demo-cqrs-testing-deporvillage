<?php

namespace Symfony\Config\Broadway;


use Symfony\Component\Config\Loader\ParamConfigurator;
use Symfony\Component\Config\Definition\Exception\InvalidConfigurationException;


/**
 * This class is automatically generated to help creating config.
 */
class SerializerConfig 
{
    private $payload;
    private $readmodel;
    private $metadata;
    
    /**
     * @default 'broadway.simple_interface_serializer'
     * @param ParamConfigurator|mixed $value
     * @return $this
     */
    public function payload($value): self
    {
        $this->payload = $value;
    
        return $this;
    }
    
    /**
     * @default 'broadway.simple_interface_serializer'
     * @param ParamConfigurator|mixed $value
     * @return $this
     */
    public function readmodel($value): self
    {
        $this->readmodel = $value;
    
        return $this;
    }
    
    /**
     * @default 'broadway.simple_interface_serializer'
     * @param ParamConfigurator|mixed $value
     * @return $this
     */
    public function metadata($value): self
    {
        $this->metadata = $value;
    
        return $this;
    }
    
    public function __construct(array $value = [])
    {
    
        if (isset($value['payload'])) {
            $this->payload = $value['payload'];
            unset($value['payload']);
        }
    
        if (isset($value['readmodel'])) {
            $this->readmodel = $value['readmodel'];
            unset($value['readmodel']);
        }
    
        if (isset($value['metadata'])) {
            $this->metadata = $value['metadata'];
            unset($value['metadata']);
        }
    
        if ([] !== $value) {
            throw new InvalidConfigurationException(sprintf('The following keys are not supported by "%s": ', __CLASS__).implode(', ', array_keys($value)));
        }
    }
    
    public function toArray(): array
    {
        $output = [];
        if (null !== $this->payload) {
            $output['payload'] = $this->payload;
        }
        if (null !== $this->readmodel) {
            $output['readmodel'] = $this->readmodel;
        }
        if (null !== $this->metadata) {
            $output['metadata'] = $this->metadata;
        }
    
        return $output;
    }

}
